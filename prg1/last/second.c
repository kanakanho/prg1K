...
// プロトタイプ宣言
void game_core(int win_count, int (*cards)[13], int* game_tmp);
void card_select(int (*cards)[13], int marks[4], int* i, int* j);
...
void game_core(int win_count, int (*cards)[13], int* game_tmp) {
  ...
  card_select(cards, marks, &mark, &card);
  ...
  while (1) {
    ...
    *j = rand() % 13;
    if (cards[*i][*j] != 0) {
      *i = marks[*i];
      *j += 1;
      char mark = what_mark(*i);
      char card = what_card(cards[*i][*j]);
      printf("%c%c\n", mark, card);
      // 引いたカードの値を消す
      cards[*i][*j] = 0;
      break;
